...
import { PriseElectriqueForm as PriseElectriqueFormType } from '@/types/electrical';

interface PriseElectriqueFormProps {
  initialData?: Partial<PriseElectriqueFormType>;
  onSubmit: (data: PriseElectriqueFormType) => void;
  onCancel: () => void;
  isEditing?: boolean;
...
export function PriseElectriqueForm({ 
  ...
  const [formData, setFormData] = useState<PriseElectriqueFormType>({
    id: initialData?.id,
  ...
  const handleInputChange = (field: keyof PriseElectriqueFormType, value: any) => {
    setFormData(prev => ({
      ...prev,
      [field]: value
    }));
  ...
  return (
    <form onSubmit={handleSubmit} className="p-6 space-y-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-lg flex items-center gap-2">
            ...
            {isEditing ? 'Modifier la prise électrique' : 'Nouvelle prise électrique'}
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <Input
                ...
                placeholder="Ex: P001, PRISE-SALON-01..."
                required
              />
            </div>
          ...
          <div className="flex items-center space-x-2">
            <Checkbox
              ...
              onCheckedChange={(checked) => handleInputChange('avec_terre', checked)}
            />
            <Label htmlFor="avec_terre">Prise avec terre</Label>
          </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-lg">Photos de la prise</CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
      ...
      <div className="flex justify-end gap-3 pt-4 border-t">
        ...
        <Button type="submit" className="flex items-center gap-2">
          ...
          {isEditing ? 'Modifier' : 'Créer'} la prise
        </Button>
      </div>
    </form>
  );
